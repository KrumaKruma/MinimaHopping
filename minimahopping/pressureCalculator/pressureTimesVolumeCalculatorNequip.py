...
_nequip_calculator = None

class EnthalpyCalculatorNequip(Calculator):
    """
    ...
    def __init__(self, base_calculator: Calculator, pressure_giga_pascale: float = 0.0):
        super().__init__()
        global _nequip_calculator
        if _nequip_calculator is not None:
            print("This class can only be constructed once. quitting.")
            quit()
        _nequip_calculator = base_calculator
    ...
    def calculate(
        ...
        if False in atoms.pbc:
            ...
            if 'energy' in properties:
                self.results['energy'] = _nequip_calculator.get_potential_energy(atoms)
            if 'forces' in properties:
                self.results['forces'] = _nequip_calculator.get_forces(atoms)

        else:
            if 'stress' in properties:
                self.results['stress'] = _nequip_calculator.get_stress(atoms) + np.array([self.pressure, self.pressure, self.pressure, .0, .0, .0])
            if 'energy' in properties:
                self.results['energy'] = _nequip_calculator.get_potential_energy(atoms) +  self.pressure * atoms.get_volume()
            if 'forces' in properties:
                self.results['forces'] = _nequip_calculator.get_forces(atoms)
