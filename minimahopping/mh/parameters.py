...
class minimaHoppingParameters:
    ...
    maxnatsphere: int = 100                           # Truncation of the OMFP length 
    ...
    fmax: float = 0.001                               # optimization stop criterion 
    enhanced_feedback:bool = False                    # enhanced feedback (rise temperature according to T = T * beta_increase * (1. + 1. * ln(n_visits)))
    energy_threshold: float = 0.0001                  # if the energy difference of two structures is below the fingerprint is compared
    output_n_lowest_minima: int = 20                  # outputs the n lowest minima
    fingerprint_threshold: float = 1e-3               # OMFP distance threshold for distinguishing minima
